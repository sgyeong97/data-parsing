...
def set_dir():
    ...
    for x in range(0, len(file_name)):
        ...
        if not(os.path.exists(temp)):
            ...
            if os.listdir(temp): #디렉터리가 있고 내부 파일이 있는 경우 실행
                temp1 = os.listdir(temp)[-1]
                temp2 ,temp1_type = os.listdir(temp)[-1].split('.')
                temp_count = temp2[-3:]
                #디렉터리 하위에 150장의 이미지가 있는 경우를 체크
                if temp_count == '150' and temp1_type == 'jpg':
                    sampling_bool.append(True)
            else: #디렉터리가 있지만 내부 파일이 없거나 디렉터리가 없는 경우 실행
                sampling_bool.append(False)
        img_path.append(temp)

def sampling():
    for x in range(0, len(file_name)):
        if not (sampling_bool[x]):
            ...
            while ret:
                ...
                multiplier = length/300

                # sampling image를 150장으로 맞추는 구문
                # count<151로 overflow 시도 방지
                if frameId % multiplier <= 1 and count < 301:
                    if count <= 150:
                        str_count = "{0:03d}".format(count)
                        temp = os.path.join(img_path[x], file_name[x]+'_'+str_count+'.jpg')
                    elif count >= 151 :
                        if count2 == 1:
                            temp_path = 'D:\신지영_업무\dataset\\0707_성남\분류\Seconds'
                            second_dir = os.path.join(temp_path, file_name[x]+'_2')
                            os.mkdir(second_dir)
                        str_count = "{0:03d}".format(count2)
                        temp = os.path.join(second_dir, file_name[x]+'_2_'+str_count+'.jpg')
                        count2 += 1
    ...
    file_path = %s""")%(file_name[x], mp4_path[x]))
            ...
            print("%s 디렉터리가 이미 있습니다" % file_name[x])
